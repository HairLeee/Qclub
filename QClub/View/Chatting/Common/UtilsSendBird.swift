...
//        var startIndex = 0
        ...
        let dumpedMessagesHash = UtilsSendBird.sha256(string: dumpedMessages)
        ...
        let messageFileNamePrefix = UtilsSendBird.sha256(string: String(format: "%@_%@", (SBDMain.getCurrentUser()?.userId.urlencoding())!, channelUrl))
    ...
    static func loadMessagesInChannel(channelUrl: String) -> [SBDBaseMessage] {
        ...
        let documentsDirectory = paths[0] as NSString
        let appIdDirectory = documentsDirectory.appendingPathComponent(SBDMain.getApplicationId()!) as NSString
        let messageFileNamePrefix = UtilsSendBird.sha256(string: String(format: "%@_%@", (SBDMain.getCurrentUser()?.userId.urlencoding())!, channelUrl))! as NSString
    ...
    static func dumpChannels(channels: [SBDBaseChannel]) {
        ...
        let dumpedChannelsHash = UtilsSendBird.sha256(string: dumpedChannels)
        ...
        let channelFileNamePrefix = UtilsSendBird.sha256(string: String(format: "%@_channellist", (SBDMain.getCurrentUser()?.userId.urlencoding())!))
    ...
    static func loadGroupChannels() -> [SBDGroupChannel] {
        let paths = NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.documentDirectory, FileManager.SearchPathDomainMask.userDomainMask, true)
        let documentsDirectory = paths[0] as NSString
        let messageFileNamePrefix = UtilsSendBird.sha256(string: String(format: "%@_channellist", (SBDMain.getCurrentUser()?.userId.urlencoding())!))! as NSString
        let dumpFileName = String(format: "%@.data", messageFileNamePrefix) as NSString
        let appIdDirectory = documentsDirectory.appendingPathComponent(SBDMain.getApplicationId()!) as NSString
        let dumpFilePath = appIdDirectory.appendingPathComponent(dumpFileName as String)
    ...
    static func findBestViewController(vc: UIViewController) -> UIViewController? {
        if vc.presentedViewController != nil {
            return UtilsSendBird.findBestViewController(vc: vc.presentedViewController!)
        }
        else if vc.isKind(of: UISplitViewController.self) {
            let svc = vc as! UISplitViewController
            if svc.viewControllers.count > 0 {
                return UtilsSendBird.findBestViewController(vc: svc.viewControllers.last!)
            }
            else {
                return vc
            }
            ...
            if svc.viewControllers.count > 0 {
                return UtilsSendBird.findBestViewController(vc: svc.topViewController!)
            }
            else {
                return vc
            }
            ...
            if (svc.viewControllers?.count)! > 0 {
                return UtilsSendBird.findBestViewController(vc: svc.selectedViewController!)
            }
            else {
                return vc
            }
